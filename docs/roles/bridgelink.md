...
does. `site.yml` orders them that way by default, and if `linumed_base_roles` selects
`bridgelink` without `monitoring` while the exporter is enabled, a preflight in this role
aborts with a clear message rather than letting Compose fail on a missing external network
(issue #86).
